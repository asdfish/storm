...
impl LogLevel {
    ...
    /// higher than [self].
    fn log<F: FnOnce(&mut dyn Write) -> io::Result<()>>(&self, level: Self, file: &mut dyn Write, print: F) {
        if level >= *self {
    ...
    pub fn execute_commands(&self) {
        self.commands
            ...
            .for_each(|command| match Command::new(command).spawn() {
                Ok(_) => {}
                Err(err) => self.log(LogLevel::Quiet, |f| writeln!(f, "error spawning command `{}`: {}", command, err))
            })
    }
